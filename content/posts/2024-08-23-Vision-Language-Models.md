...
<p align="justify">
<b>Zero-shot Image Classification</b>: Involves evaluating a model on a classification task for which it hasn't been explicitly trained. This approach has gained traction, particularly with the use of prompt engineering. By incorporating concept names within carefully crafted, human-engineered prompt templates, the model's zero-shot performance can be significantly improved. Recent advancements have introduced methods that utilize LLMs to automatically generate prompts, often enriched with detailed visual descriptions. The effectiveness of zero-shot classification largely depends on whether the training data includes examples that represent the respective classes, as models rely on their prior knowledge to make accurate predictions in these tasks.
</p>
