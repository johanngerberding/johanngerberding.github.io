...
{{< figure align=center alt="YOLO model architecture" src="/imgs/object_detection_2/yolo-network-architecture.png" width=100% caption="Figure 1. YOLO network architecture">}}
...
$$
L\_{loc} = \lambda_{coord} \sum_{i=0}^{S^{2}} \sum_{j=0}^{B} \mathbb{1}\_{ij}^{obj} [(x\_{i} - \hat{x}\_{i})^{2} + (y\_{i} - \hat{y}\_{i})^{2} + (\sqrt{w\_{i}} - \sqrt{\hat{w}\_{i}})^{2} + (\sqrt{h\_{i}} - \sqrt{\hat{h}\_{i}})^{2}]
$$

$$
L\_{cls} = \sum_{i=0}^{S^{2}} \sum_{j=0}^{B} (\mathbb{1}\_{ij}^{obj} \lambda_{noobj}(1 - \mathbb{1}\_{ij}^{obj})) (C\_{i} - \hat{C}\_{i})^{2} + \sum_{i=0}^{S^{2}} \sum_{C=C} \mathbb{1}\_{i}^{obj} (p\_{i}(c) - \hat{p}\_{i}(c))^{2}
$$
...
$C\_{i}$ | confidence score of cell $i$, $Pr(contains object) * IoU (pred, truth)$
...
$p\_{i}$ | conditional probability of whether cell $i$ contains an object of class $c \in \mathcal{C}$
...
**Batch Normalization:** This leads to significant improvements in convergence while eliminating the need for other forms of regularization like dropout (+2% mAP).
...
$$
d(x, c\_{i}) = 1 - IoU(x, c\_{i}), \quad i=i,...k
$$

where $x$ is a ground truth box candidate and $c\_{i}$ is one of the centroids / the closest centroid.

**Direct Location Prediction:** In Region Proposal Networks the box location prediction is unconstrained which means any anchor box can end up at any point in the image which can lead to an unstable training. YOLOv2 follows the approach of the original YOLO model by predicting location coordinates relative to the location of the grid cell (using a logistic activation). Given the anchor box width $p\_{w}$ and height $p_{h}$ in the grid cell with the top left corner ($c\_{x}, c\_{y}$) the model predicts 5 values ($t\_{x}, t\_{y}, t\_{w}, t\_{h}, t\_{o}$) which correspond to the following box values:
...
b\_{w} = p\_{w}e^{t\_{w}} \\
b\_{h} = p\_{h}e^{t\_{h}} \\
Pr(obj) =IoU(b, obj) = \sigma (t\_{o})
$$

This in combination with clustering priors improves mAP by up to 5%.

{{< figure align=center alt="YOLOv2 bounding box prediction format" src="/imgs/object_detection_2/yolov2-loc.png" width=60% caption="Figure 2. YOLOv2 bounding box prediction">}}
...
$$
FL(p\_{t}) = - (1 - p\_{t})^{\gamma} \log (p\_{t})
$$

{{< figure align=center alt="Focal Loss with different gamma values" src="/imgs/object_detection_2/focal-loss.png" width=70% caption="Figure 3. Focal Loss with different gamma values">}}
...
$$
FL(p\_{t}) = - \alpha (1 - p\_{t})^{\gamma} \log (p\_{t})
$$
